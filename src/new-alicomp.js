...
async function main(uri) {
  ...
  let packName = await util.getPackageName("COMPONENT")
  if (!packName) return
  ...
  let { toolKebabCase, toolPascalCase } = util.getToolerName(packName, "COMPONENT")

  let sourceFolder = path.join(__dirname, "template", "template-ali-comp")
  ...
  try {
    ...
    fileData = fileData.replace(/XxHelper/g, toolPascalCase)
    fileData = fileData.replace(/xx-helper/g, toolKebabCase)
  ...
  // 处理 axml 文件
  try {
    s1 = path.join(sourceFolder, "index.axml")
    fileData = fs.readFileSync(s1, 'utf-8')
    fileData = fileData.replace(/xxx-index/g, packName)
    s2 = path.join(targetFolder, `${packName}.axml`)

    await writeFile(s2, fileData)

  } catch (err) {
    console.log("出现 读写 axml 文件错误......")
    console.log(err)
  }
  ...
  // 处理 acss 文件
  try {
    s1 = path.join(sourceFolder, "index.acss")
    fileData = fs.readFileSync(s1, 'utf-8')
    fileData = fileData.replace(/xxx-index/g, packName)
    s2 = path.join(targetFolder, `${packName}.acss`)

    await writeFile(s2, fileData)
  } catch (err) {
    console.log("出现 读写 acss 文件错误......")
    console.log(err)
  }
  ...
  //处理 xx-helper.js 文件
  try {
    s1 = path.join(sourceFolder, "xx-helper.js")
    ...
    console.log("出现 读写 xx-helper.js 文件错误......")
    console.log(err)
  }
...
function register(context) {
  let command = vscode.commands.registerCommand("miniCreate.aliMini.createComponent", (uri) => {
    if (!uri) {
